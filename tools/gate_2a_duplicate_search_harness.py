...
from observation_workbench.reconciliation.types import InventoryObservation, RemoteRecordKey, RemoteSite  # noqa: E402
...
class _StubINatClient:
    ...
    def get_creation_duplicate_search(self, token, *, user_id, page=1, d1="", d2="", id_above=None):
...
def _service(inat_client, db) -> ObservationCreationService:
    return ObservationCreationService(
        ...
        lambda _p: "", lambda: 1, lambda: 1, None, None,
    )


def _source_inventory(with_date: bool) -> InventoryObservation:
    return InventoryObservation(
        key=RemoteRecordKey(RemoteSite.MO, 42), account_id=1, owner_id=1, owner_login="tester",
        observed_on=date(2026, 1, 1) if with_date else None, taxon_id=1, taxon_name="Amanita sp.",
        taxon_rank="species", public_locality="", fungi_status="in_scope", updated_at=None,
    )
...
def test_clean_full_search_with_date() -> None:
    ...
    try:
        service._live_search_inat_destination(_profile(), _source_inventory(True), lambda: False)  # noqa: SLF001
        ok = True
    except Exception as exc:  # noqa: BLE001
        ok = False
        print(f"  unexpected exception: {exc}")
    check("clean search (date window + full scan both empty) completes without raising", ok)
    check("full-account scan STILL RAN even though a date window search ran first", any(c["id_above"] is not None for c in inat.calls))
...
def test_dateless_destination_candidate_found_by_full_scan() -> None:
    ...
    dateless_candidate = {
        "id": 999001, "observed_on": None, "updated_at": "2026-01-01T00:00:00Z",
        "taxon": {"id": 1, "name": "Amanita sp.", "ancestor_ids": [47170]},
        "user": {"id": 555}, "place_guess": "", "observation_photos": [],
...
def test_malformed_page_shape_fails_closed() -> None:
    ...
    try:
        ...
        check("malformed 'results' shape fails closed", exc.code == "duplicate_search_unavailable", f"got {exc.code}")
...
def test_non_dict_candidate_fails_closed() -> None:
    ...
    try:
        ...
        check("non-dict candidate result fails closed", exc.code == "duplicate_search_unavailable", f"got {exc.code}")
...
def test_missing_id_field_fails_closed() -> None:
    ...
    try:
        ...
        check("candidate missing required id field fails closed", exc.code == "duplicate_search_unavailable", f"got {exc.code}")
...
def test_missing_mo_binding_blocks() -> None:
    ...
    try:
        ...
        check("missing MO reciprocal-link binding blocks creation", exc.code == "duplicate_search_unavailable", f"got {exc.code}")
...
def test_unverified_mo_binding_blocks() -> None:
    ...
    try:
        ...
        check("unverified MO reciprocal-link binding blocks creation", exc.code == "duplicate_search_unavailable", f"got {exc.code}")
...
def test_binding_load_error_blocks() -> None:
    ...
    try:
        service._live_search_inat_destination(_profile(), _source_inventory(True), lambda: False)  # noqa: SLF001
        check("MO binding DB read error blocks creation (not silently 'no evidence')", False)
    except ObservationCreationError as exc:
        check(
            "MO binding DB read error blocks creation (not silently 'no evidence')",
            exc.code == "duplicate_search_unavailable", f"got {exc.code}",
        )


def test_repeated_cursor_fails_closed() -> None:
    stuck_page = {"results": [{"id": 100, "observed_on": None, "updated_at": "x", "taxon": None,
                                "user": {"id": 555}, "place_guess": "", "ofvs": [], "observation_photos": []}] * 200}
    inat = _StubINatClient(window_pages=[{"results": []}], scan_pages=[stuck_page, stuck_page])
    service = _service(inat, _StubDB())
    try:
        service._live_search_inat_destination(_profile(), _source_inventory(True), lambda: False)  # noqa: SLF001
...
def test_dateless_source_goes_straight_to_full_scan() -> None:
    ...
    service = _service(inat, _StubDB())
    try:
        service._live_search_inat_destination(_profile(), _source_inventory(False), lambda: False)  # noqa: SLF001
        ok = True
    except Exception as exc:  # noqa: BLE001
        ok = False
        print(f"  unexpected exception: {exc}")
