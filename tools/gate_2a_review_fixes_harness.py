...
def _journal(db: ReconciliationDB, profile_id: int, mo_id: int) -> tuple[int, int, int]:
    ...
    return db.journal_observation_creation_actions(
        profile_id,
        source_site="mo",
        source_observation_id=mo_id,
        destination_site="inat",
        source_fingerprint=source_fingerprint,
        correlation_marker=f"marker-{mo_id}",
        marker_location="client_uuid_field",
        approved_field_gaps=[],
        item_specs=[],
    )
...
def _setup_finalizable(
    ...
    *,
    ...
    pair_id = db.settle_creation_write_success(
        ...
        create_action_id,
        attempt_id,
        db.creation_ledger_for_group(profile.profile_id, group_id)["creation_id"],
        group_id,
        destination_site="inat",
        destination_id=inat_id,
        destination_uuid="uuid-1",
...
def test_finalize_success() -> None:
    ...
    ok = db.settle_pair_finalize_success(
        st["profile_id"],
        st["finalize_action_id"],
        st["group_id"],
        st["pair_id"],
        mo_observation_id=st["mo_id"],
        inat_observation_id=st["inat_id"],
    )
    check("finalize: correct provenance succeeds", ok is True)
    pair = db.pair_detail(st["profile_id"], st["pair_id"])
    check(
        "finalize: pair promoted to confirmed",
        pair is not None and str(pair["review_state"]) == "confirmed",
    )
    action = db.action(st["profile_id"], st["finalize_action_id"])
    check(
        "finalize: action marked succeeded",
        action is not None and str(action["state"]) == "succeeded",
    )
    attempt_row = (
        db.connection()
        .execute(
            "SELECT state FROM sync_creation_attempts WHERE attempt_id=?",
            (st["attempt_id"],),
        )
        .fetchone()
    )
    check(
        "finalize: attempt remains succeeded", str(attempt_row["state"]) == "succeeded"
    )
    ...
    check(
        "finalize: already-finalized action id refuses (double-finalize)", ok2 is False
    )
...
def test_finalize_negatives() -> None:
    ...
    check(
        "finalize negative: wrong group leaves pair untouched",
        str(pair["review_state"]) == "provisional",
    )
    ...
    ok = st["db"].settle_pair_finalize_success(
        ...
        st["pair_id"],
        mo_observation_id=st["mo_id"],
        inat_observation_id=st["inat_id"] + 1,
    )
    ...
    st["db"].connection().execute(
        "UPDATE sync_actions SET state='pending' WHERE action_id=?",
        (st["finalize_action_id"],),
    )
    ...
    # side too).
    db, _path = _fresh_db()
    st_a = _setup_finalizable(
        db, inat_user_id=9101, mo_user_id=9102, mo_id=711111, inat_id=811111
    )
    st_b = _setup_finalizable(
        db, inat_user_id=9111, mo_user_id=9112, mo_id=911111, inat_id=911112
    )
    ok = db.settle_pair_finalize_success(
        st_a["profile_id"],
        st_b["finalize_action_id"],
        st_b["group_id"],
        st_b["pair_id"],
        mo_observation_id=st_b["mo_id"],
        inat_observation_id=st_b["inat_id"],
    )
    ...
    st = fresh_state()
    st["db"].connection().execute(
        "UPDATE sync_creation_attempts SET state='outcome_unknown' WHERE attempt_id=?",
        (st["attempt_id"],),
    )
    st["db"].connection().commit()
    ok = st["db"].settle_pair_finalize_success(
    ...
    st = fresh_state()
    st["db"].connection().execute(
        "DELETE FROM sync_creation_attempts WHERE attempt_id=?", (st["attempt_id"],)
    )
    st["db"].connection().commit()
    ok = st["db"].settle_pair_finalize_success(
        st["profile_id"],
...
def test_ambiguous_action_preservation() -> None:
    ...
    _seed_source_record(db, profile.profile_id, mo_id)
    group_id, create_action_id, attempt_id = _journal(db, profile.profile_id, mo_id)
    creation_id = db.creation_ledger_for_group(profile.profile_id, group_id)[
        "creation_id"
    ]
    inat_id = 400001
    pair_id = db.settle_creation_write_success(
    ...
    check(
        ...
        action_a_after is not None
        and str(action_a_after["state"]) == "outcome_unknown",
    ...
    group_id2, create_action_id2, attempt_id2 = _journal(
        db2, profile2.profile_id, mo_id2
    )
    creation_id2 = db2.creation_ledger_for_group(profile2.profile_id, group_id2)[
        "creation_id"
    ]
    inat_id2 = 600001
    pair_id2 = db2.settle_creation_write_success(
        ...
        group_id2,
        destination_site="inat",
        destination_id=inat_id2,
        destination_uuid="uuid-3",
        source_record_id=mo_id2,
    )
    item_b = (
        db2.connection()
        .execute(
    ...
    item_b_after = [
        i
        for i in db2.creation_items(profile2.profile_id, attempt_id2)
        if i["creation_item_id"] == item_b
    ][0]
    check(
...
def _setup_confirmed_population(mo_id: int, inat_id: int, seed: int) -> dict:
    ...
    profile = db.save_profile(
        9500 + seed, f"inat_cancel{seed}", 9600 + seed, f"mo_cancel{seed}"
    )
    _seed_source_record(db, profile.profile_id, mo_id)
    group_id, create_action_id, attempt_id = _journal(db, profile.profile_id, mo_id)
    creation_id = db.creation_ledger_for_group(profile.profile_id, group_id)[
    ...
    pair_id = db.settle_creation_write_success(
        ...
        destination_site="inat",
        destination_id=inat_id,
        destination_uuid=f"uuid-cancel{seed}",
        source_record_id=mo_id,
    )
    now = "2026-01-01T00:00:00Z"
    db.connection().execute(
...
class _StubPhotoService:
    ...
    def _refresh(self, profile, pair, cancelled):  # noqa: ANN001
        ...
        return SimpleNamespace(
            source_photos=[self._source],
            inat_record_fingerprint="ifp",
            mo_record_fingerprint="mfp",
        )
...
def test_preflight_atomicity_positive_cases() -> None:
    ...
    check(
        "preflight positive: no-existing-action case mints and fails atomically",
        outcome["downgraded"] is True and outcome["state"] == "failed",
        f"got {outcome}",
    )
    action = db.action(st["profile_id"], outcome["action_id"])
    check(
        "preflight positive: the newly-minted action itself is 'failed'",
        action is not None and str(action["state"]) == "failed",
    )
    item_after = [
        i
        for i in db.creation_items(st["profile_id"], st["attempt_id"])
        if i["creation_item_id"] == st["item_id"]
    ][0]
    check(
        "preflight positive: the creation item itself is 'failed'",
        str(item_after["state"]) == "failed",
    )
    ...
    pending_action_id = db2.mint_creation_item_action(
        ...
        "inat_photo_attach",
        pair_id=st2["pair_id"],
        mo_observation_id=st2["mo_id"],
        inat_observation_id=st2["inat_id"],
        site="inat",
    )
    outcome2 = db2.fail_creation_item_preflight(
        ...
        st2["item_id"],
        "inat_photo_attach",
        reason="pending with no write started",
        next_ordinal=5,
        pair_id=st2["pair_id"],
        mo_observation_id=st2["mo_id"],
        inat_observation_id=st2["inat_id"],
    ...
    check(
        ...
        outcome2["downgraded"] is True
        and outcome2["state"] == "failed"
        and outcome2["action_id"] == pending_action_id,
        f"got {outcome2}",
    )
...
def test_preflight_atomicity_race_never_downgrades() -> None:
    ...
    outcome = db.fail_creation_item_preflight(
        ...
        st["item_id"],
        "inat_photo_attach",
        reason="simulated preflight failure racing a concurrent in-flight write",
        next_ordinal=5,
        pair_id=st["pair_id"],
        mo_observation_id=st["mo_id"],
        inat_observation_id=st["inat_id"],
    ...
    check(
        "preflight race: refuses to downgrade a running/write-started action",
        outcome["downgraded"] is False,
        f"got {outcome}",
    )
    check(
        "preflight race: reports the REAL state (running), not 'failed'",
    ...
    outcome2 = db.fail_creation_item_preflight(
        ...
        st["item_id"],
        "inat_photo_attach",
        reason="second simulated preflight failure racing outcome_unknown",
        next_ordinal=6,
        pair_id=st["pair_id"],
        mo_observation_id=st["mo_id"],
        inat_observation_id=st["inat_id"],
...
def test_population_loop_cancellation_before_download() -> None:
    ...
    source = SimpleNamespace(
        ...
        source_url="stub://cancel-before-download",
        license_label=st["license_label"],
        copyright_holder=st["holder"],
    )
    ...
    check(
        "cancel-before-download: tail stops at exactly one item",
        len(results) == 1,
    ...
    check(
        "cancel-before-download: the full-image download was never attempted",
        stub.download_calls == 0,
    )
    item_after = [
        i
        for i in db.creation_items(st["profile_id"], st["attempt_id"])
    ...
    check(
        "cancel-before-download: item durably recorded 'failed'",
        str(item_after["state"]) == "failed",
    )
    check(
        "cancel-before-download: item got a real linked action (not a bare -1/0 pseudo id)",
        item_after.get("action_id") is not None,
    )
    if item_after.get("action_id") is not None:
        ...
        check(
            ...
            action_after is not None
            and str(action_after["state"]) not in ("running", "outcome_unknown"),
            f"got state={action_after['state'] if action_after else None}",
        )
...
def test_finalize_pair_id_provenance_mismatch() -> None:
    ...
    db, _path = _fresh_db()
    st = _setup_finalizable(db)
    creation_id = db.creation_ledger_for_group(st["profile_id"], st["group_id"])[
        "creation_id"
    ]
    ...
    try:
        ...
        raised = True
    check(
        "finalize negative: sync_created_observations.pair_id mismatch refuses (raises)",
        raised,
    )
    pair = db.pair_detail(st["profile_id"], st["pair_id"])
    check(
