...
from observation_workbench.reconciliation.mo_client import MOAPIError, MOClient  # noqa: E402
from observation_workbench.reconciliation.mo_parsing import parse_mo_observation  # noqa: E402
from observation_workbench.reconciliation.observation_creation import (  # noqa: E402
    ObservationCreationError, ObservationCreationService,
)
...
class LostResponseINat(INatClient):
    ...
    def create_observation_v2(self, *args, **kwargs):  # type: ignore[override]
        ...
        raise INatAPIError(
            ...
            endpoint="/observations", request_phase="unsafe_write", method="POST",
        )
...
class LostResponseMO(MOClient):
    def create_observation(self, *args, **kwargs):  # type: ignore[override]
        ...
        raise MOAPIError(
            "observations", None, "Simulated lost response (--simulate-lost-response).",
            response_received=False, outcome_unknown=True, error_code="simulated_lost_response",
        )
...
def run(args: argparse.Namespace) -> int:
    ...
    who_payload = inat_client._request_v2_auth(  # noqa: SLF001 - harness-only introspection
        "GET", "/users/me", inat_jwt, params={"fields": "(id:!t,login:!t)"},
    )
    ...
    if args.source_site == "inat":
        raw = _first(inat_client.get_reconciliation_detail(args.source_id, inat_jwt, deep=True))
        if not raw:
            print(f"iNaturalist observation {args.source_id} is not readable.", file=sys.stderr)
            return 1
        ...
        if source_owner_id != inat_user_id:
            ...
            return 1
        inventory = INatReconciliationReader(inat_client).parse_inventory(raw, inat_user_id, None, None)
        if not args.mo_login:
            print("--mo-login is required when the destination is Mushroom Observer.", file=sys.stderr)
            return 2
        mo_user = mo_client.resolve_user(args.mo_login, cancelled)
        if not mo_user:
            print(f"Could not resolve Mushroom Observer login {args.mo_login!r}.", file=sys.stderr)
            return 2
        mo_user_id = int(mo_user["id"])
        mo_login = str(mo_user.get("login_name") or mo_user.get("login") or args.mo_login)
    else:
        raw = _first(mo_client.observation(args.source_id, cancelled, detail="high"))
        if not raw:
            print(f"Mushroom Observer observation {args.source_id} is not readable.", file=sys.stderr)
            return 1
        owner = raw.get("owner") or {}
        mo_user_id = int(owner.get("id") or 0)
        mo_login = str(owner.get("login_name") or "")
        if not mo_user_id:
            print(f"Mushroom Observer observation {args.source_id} has no readable owner id.", file=sys.stderr)
            return 1
    ...
    print(f"Authenticated: iNat {inat_login!r} (id {inat_user_id}); MO {mo_login!r} (id {mo_user_id})")

    if not args.run:
        print("\nDRY RUN — nothing was sent. Re-run with --run --i-own-these-records to execute.")
        print(f"Would create a destination observation from {args.source_site}:{args.source_id}, ")
        print("run the full saga (create -> per-item rows -> reciprocal links -> pair_finalize),")
        if args.with_photo_item:
            print("also select and upload one real photo population item,")
        print("then delete the created observation as cleanup" + (
            ", and prove outcome_unknown recovery without a second create." if args.simulate_lost_response else "."
        ))
        return 0
    if not args.i_own_these_records:
        print("Refusing to write. --run requires --i-own-these-records.", file=sys.stderr)
        return 2
...
def _run_saga_with_db(
    ...
    mo_field_defs = INatReconciliationReader(inat_client).resolve_field_definitions("Mushroom Observer URL")
    ...
    db.save_field_binding(
        profile.profile_id, "mo_url", int(mo_field_defs[0]["id"]), "Mushroom Observer URL",
        "text", "verified",
    )
    ...
    auth_provider = lambda: AuthState(api_token=inat_jwt, login=inat_login)  # noqa: E731
    mo_key_provider = lambda _profile_id: mo_key  # noqa: E731
    gen_provider = lambda: 1  # noqa: E731
    photo_service = PhotoSyncService(db, inat_client, mo_client, auth_provider, mo_key_provider, gen_provider, gen_provider)
    link_service = LinkRepairService(db, inat_client, mo_client, auth_provider, mo_key_provider, gen_provider, gen_provider)
    service = ObservationCreationService(
        db, inat_client, mo_client, auth_provider, mo_key_provider, gen_provider, gen_provider,
        photo_service, link_service,
    )

    try:
        preview = service.prepare_preview(profile.profile_id, args.source_site, args.source_id, cancelled=cancelled)
    except ObservationCreationError as exc:
        print(f"prepare_preview FAILED: {exc}", file=sys.stderr)
        return 1
    print(f"Preview built: destination {preview.destination_site.value}, "
    ...
    if args.with_photo_item:
        candidate = next(
            (item for item in preview.items if item.item_type == "photo" and item.enabled), None,
        )
        if not candidate:
            print(
        ...
        print(f"Selected photo item {candidate.source_identity!r} for population (--with-photo-item).")

    import uuid as uuidlib
    marker = str(uuidlib.uuid4()) if preview.destination_site.value == "inat" else f"[gate2a-saga-harness:{uuidlib.uuid4()}]"
    ...
    group_id, create_action_id, attempt_id = db.journal_observation_creation_actions(
        ...
        reviewed_source_taxon_name=preview.taxon_name, reviewed_source_taxon_rank=preview.taxon_rank,
    ...
    print(f"Journaled group {group_id}, create action {create_action_id}, creation attempt {attempt_id}.")
    ...
    for result in results:
        print(f"  result: action {result.action_id} -> {result.state}: {result.message}")
    ...
    if create_row and str(create_row["state"]) == "outcome_unknown":
        if args.simulate_lost_response:
            ...
            recovery = service.verify_unknown(profile.profile_id, create_action_id, cancelled)
            print(f"  verify_unknown: {recovery.state}: {recovery.message}")
            if recovery.state != "succeeded":
                print("RECOVERY FAILED — the create may be an orphan. Check manually.", file=sys.stderr)
                return 1
            create_row = db.action(profile.profile_id, create_action_id)
        else:
            print("Create ended in outcome_unknown unexpectedly.", file=sys.stderr)
    if create_row:
        destination_id = int(create_row["mo_observation_id"] or 0) if destination_site == "mo" else int(create_row["inat_observation_id"] or 0)
    ...
    population_note = (
        ...
        if item_specs else
        "NOTE: item_specs=[] means NO population item (photo) was exercised by this run; do not "
    ...
    print("RESULT: " + (
        ...
        if all_succeeded else "INCOMPLETE — see rows above."
    ))
    ...
    if not args.no_cleanup and destination_id:
        ...
        if destination_site == "inat":
            uuid_row = create_row
            obs_uuid = str(uuid_row["inat_observation_uuid"]) if uuid_row and uuid_row["inat_observation_uuid"] else ""
            if not obs_uuid:
                detail = _first(inat_client.get_reconciliation_detail(destination_id, inat_jwt, deep=True))
                obs_uuid = str(detail.get("uuid") or "")
            if obs_uuid:
                deleted = inat_client.delete_observation_v2(inat_jwt, obs_uuid)
                print(f"  DELETE /observations/{obs_uuid}: HTTP {deleted.metadata.status_code}")
            else:
                print(f"  Could not resolve a uuid for observation {destination_id}; delete manually.")
        else:
            try:
                mo_client._write(  # noqa: SLF001 - harness-only cleanup; no public delete exists (deliberately, per the saga's never-auto-delete design)
                    "DELETE", "observations", {"api_key": mo_key, "id": destination_id}, cancelled,
                )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--source-id", type=int, required=True, help="A DISPOSABLE observation you own")
    parser.add_argument(
        "--mo-login", default="",
    ...
    parser.add_argument("--simulate-lost-response", action="store_true",
                         help="Send the real create, then force outcome_unknown and prove verify_unknown recovers it")
    parser.add_argument("--with-photo-item", action="store_true",
