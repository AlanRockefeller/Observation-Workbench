...
from .types import (
    CandidateScore, EvidenceFamily, EvidenceTier, FAMILY_CAPS, HydratedObservation,
    InventoryObservation, MatchEvidence, ObservationPair, RemoteSite,
)
...
CANDIDATE_IDENTIFIER_KINDS = frozenset({"voucher", "collection", "collection_number", "field_number"})
...
def score_evidence(items: Iterable[MatchEvidence]) -> CandidateScore:
    ...
    capped = {family: min(score, FAMILY_CAPS[family]) for family, score in grouped.items()}
    total = sum(capped.values())
    # Barcode is useful corroboration but cannot be the independent basis of strength.
    non_barcode_families = {family for family, score in capped.items() if score and family != EvidenceFamily.BARCODE}
    strong = total >= 70 and len({family for family, score in capped.items() if score}) >= 2 and bool(non_barcode_families)
...
def build_candidates(
    mo_records: Iterable[InventoryObservation], inat_records: Iterable[InventoryObservation],
) -> list[ObservationPair]:
    """Join inverted blocking indexes; never scan every opposite record."""
    mo = [
        item for item in mo_records
        if item.fungi_status in {"fungi", "unknown"}
        and item.scope_state == "in_scope" and not item.deleted
    ]
    inat = [
        item for item in inat_records
    ...
    for item in inat:
        ...
        if item.observed_on:
            ...
            if locality:
                date_locality_index[(item.observed_on, locality)].add(item.key.observation_id)
    ...
    for left in mo:
        if left.observed_on:
            ...
            for delta in (-1, 0, 1):
                ...
                if locality:
                    candidate_ids.update(date_locality_index.get((observed, locality), ()))
                blocks.update((left.key.observation_id, value) for value in candidate_ids)
    ...
    for mo_id, inat_id in sorted(blocks):
        ...
        results.append(ObservationPair(mo_id, inat_id, link_state, score.total, evidence=score.evidence))
    return results


def score_candidate(
    left: InventoryObservation, right: InventoryObservation,
    extra: Iterable[MatchEvidence] = (), *, explicit_review: bool = False,
    ...
    return ObservationPair(
        left.key.observation_id, right.key.observation_id,
        authoritative_link_state(left, right), score.total, evidence=score.evidence,
    )


def _inventory_evidence(left: InventoryObservation, right: InventoryObservation) -> tuple[MatchEvidence, ...]:
    evidence: list[MatchEvidence] = []
    linked = right.key.observation_id in left.authoritative_targets or left.key.observation_id in right.authoritative_targets
    if linked:
        evidence.append(MatchEvidence("authoritative_link", EvidenceFamily.LINK, 65,
    ...
    if left.observed_on and right.observed_on:
        ...
        if difference == 0:
            evidence.append(MatchEvidence("exact_date", EvidenceFamily.TEMPORAL, 15, "Observed dates are equal."))
        elif difference == 1:
            evidence.append(MatchEvidence("adjacent_date", EvidenceFamily.TEMPORAL, 5, "Observed dates are adjacent."))
    if _same_locality(left, right):
        evidence.append(MatchEvidence("same_public_locality", EvidenceFamily.SPATIAL, 3,
    ...
    if left.taxon_name and right.taxon_name:
        if left.taxon_name.casefold() == right.taxon_name.casefold():
            evidence.append(MatchEvidence("exact_taxon", EvidenceFamily.TAXON, 10, "Taxon names match."))
        elif _same_genus(left.taxon_name, right.taxon_name):
            evidence.append(MatchEvidence("same_genus", EvidenceFamily.TAXON, 5, "Taxa share a genus."))
    ...
    for kind, value in sorted(left_identifiers.intersection(right.identifiers)):
        family = EvidenceFamily.BARCODE if kind == "accession" else EvidenceFamily.SPECIMEN
        points = 35 if family == EvidenceFamily.BARCODE else 50
        evidence.append(MatchEvidence(f"exact_{kind}", family, points, f"Exact normalized {kind} matches.", EvidenceTier.METADATA))
    ...
    if left_sources.intersection(right_sources):
        evidence.append(MatchEvidence("native_media_identity", EvidenceFamily.MEDIA, 60,
                                      "Both records identify the same explicit source-qualified media.", EvidenceTier.METADATA))
    return tuple(evidence)


def authoritative_link_state(left: InventoryObservation, right: InventoryObservation) -> str:
    ...
    *, include_coordinates: bool = True,
    ...
    for record in (left.inventory, right.inventory):
        if record.owner_id != record.account_id:
            conflicts.append(f"{record.key.site.value} owner differs from the selected profile account.")
        if record.fungi_status == "nonfungal":
            conflicts.append(f"{record.key.site.value} record is outside kingdom Fungi.")
        ...
        if record.deleted or record.availability_state == "deleted":
            conflicts.append(f"{record.key.site.value} record is deleted or unavailable.")
    ...
    if include_coordinates and left.coordinates_available and right.coordinates_available:
        if None not in (left.latitude, left.longitude, right.latitude, right.longitude):
            ...
            tolerance = max(10_000.0, (left.accuracy_m or 0.0) + (right.accuracy_m or 0.0))
            if distance > tolerance:
                conflicts.append("Available points exceed 10 km and their combined accuracy tolerance.")
    return tuple(conflicts), tuple(unavailable)


def validate_reciprocal_pair(left: HydratedObservation, right: HydratedObservation) -> tuple[str, tuple[str, ...]]:
    """Complete every material check before a reciprocal pair can auto-confirm."""
    if authoritative_link_state(left.inventory, right.inventory) != "reciprocal_unvalidated":
        return "ambiguous_link", ("The authoritative links are not uniquely reciprocal.",)
    if not left.required_values_available or not right.required_values_available:
        return "ambiguous_link", ("A required validation value is hidden or unavailable.",)
...
def is_same_site_duplicate(
    first: InventoryObservation, second: InventoryObservation, *, shared_strong_opposite: bool = False,
) -> bool:
    ...
    same_native_photo = bool(first_media.intersection((item.site, item.photo_id) for item in second.media))
    return bool(same_native_photo and first.observed_on == second.observed_on and _taxon_compatible(first, second))


def _same_locality(left: InventoryObservation, right: InventoryObservation) -> bool:
    return bool(_normalized_locality(left.public_locality) and
                _normalized_locality(left.public_locality) == _normalized_locality(right.public_locality))


def _same_genus(left: str, right: str) -> bool:
    return bool(_normalized_genus(left) and _normalized_genus(left) == _normalized_genus(right))
...
def _taxon_compatible(left: InventoryObservation, right: InventoryObservation) -> bool:
    return bool(left.taxon_name and right.taxon_name and
                (left.taxon_name.casefold() == right.taxon_name.casefold() or _same_genus(left.taxon_name, right.taxon_name)))
